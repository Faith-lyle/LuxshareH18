...
if __name__ == '__main__': 
    ...
    parser.add_argument('-s', '--switch', help='Switch GN power path [on|off].', type=str, default=False, required=True)
    ...
    if 'on' in state: gnTest.enableCharging(True)
    if 'off' in state: gnTest.enableCharging(False)
