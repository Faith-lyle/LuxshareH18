...
if __name__ == '__main__': 
    ...
    if 'on' == switch: 
        ...
        # Add forceDFU to exit locust LP mode in user App
        gnTest.forceDFU(on=True)
        gnTest.hostReset(on=True)
        gnTest.hostReset(on=False)
        gnTest.KIS2Locust(on=True)
        # Add 2nd forceDFU for Goldrestore
        gnTest.hostReset(on=True)
        gnTest.forceDFU(on=True)
        gnTest.hostReset(on=False)
        gnTest.forceDFU(on=False)
        print('GN host: KIS FWDL path enabled; Use /dev/cu.kis-xxxxxxxx-ch-0 for goldrestore')
    if 'off' == switch:
        gnTest.PMUReset()
        time.sleep(1)
        gnTest.resetAll()
